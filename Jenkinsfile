                ...
                stage('Run movie service') {
                    steps {
                        sh '''
                            ...
                            docker run --name movie_service -d -p 8001:8000 -e DATABASE_URI=postgresql://movie_db_username:movie_db_password@movie_db/movie_db_dev "$DOCKER_ID/$DOCKER_IMAGE_MOVIE_SERVICE:$DOCKER_TAG"
                            sleep 6
                ...
                stage('Run cast service') {
                    steps {
                        sh '''
                            ...
                            docker run --name cast_service -d -p 8002:8000 -e DATABASE_URI=postgresql://cast_db_username:cast_db_password@cast_db/cast_db_dev "$DOCKER_ID/$DOCKER_IMAGE_CAST_SERVICE:$DOCKER_TAG"
                            sleep 6
